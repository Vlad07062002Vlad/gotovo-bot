...
try:
    ...
    async def _search_rules(client, query, subject_key, grade, top_k=5):
        return []
...
# ---------- Гибридный роутер моделей (может быть подменён сервисом) ----------
...
async def call_model(uid: int, user_text: str, mode: str) -> str:
    ...
    try:
        ...
        out_text = ("❌ Не получилось получить ответ от модели. "
...
async def call_model_followup(uid: int, prev_task: str, prev_answer: str, follow_q: str, mode_tag: str) -> str:
    ...
    prompt = (
        ...
        f"Вопрос-уточнение:\n{follow_q[:1200]}\n\n"
        "Дай только дополнение/уточнение, не переписывай всё решение."
    )
...
# Если сервисов нет — НИКАКИХ SystemExit. Делаем живые заглушки.
try:
    from services.usage import consume_request, my_stats, daily_summary, get_user_plan, add_credits, activate_sub  # type: ignore
    from services.payments import build_buy_keyboard, apply_payment_payload, get_stars_amount as _get_stars_amount_ext  # type: ignore
    from services.router import select_model as _select_model_ext  # type: ignore
    if _select_model_ext:
        select_model = _select_model_ext  # подмена эвристики
    log.info("services/* connected")
except Exception as e:
    log.info(f"services/* not connected, using stubs: {e}")

    # Простые in-memory лимиты: Free 3/день + 1 Trial/день; кредиты/подписка отсутствуют.
    _DAY = lambda: int(time.time() // 86400)
    _COUNTS = defaultdict(lambda: {"day": _DAY(), "free": 0, "trial": 0})
    def _roll(uid: int):
        d = _COUNTS[uid]
        if d["day"] != _DAY():
            d["day"] = _DAY(); d["free"] = 0; d["trial"] = 0

    def get_user_plan(uid: int):
        _roll(uid)
        left_free = max(0, 3 - _COUNTS[uid]["free"])
        left_trial = max(0, 1 - _COUNTS[uid]["trial"])
        return {"free_left_today": left_free, "trial_left_today": left_trial, "sub_active": False, "sub_left_month": 0, "credits": 0}

    def consume_request(uid: int, need_pro: bool, allow_trial: bool):
        _roll(uid)
        if need_pro:
            if allow_trial and _COUNTS[uid]["trial"] < 1:
                _COUNTS[uid]["trial"] += 1
                return True, "trial", ""
            return False, "free", "нет Trial/Pro"
        # free
        ...
        if allow_trial and _COUNTS[uid]["trial"] < 1:
            _COUNTS[uid]["trial"] += 1
            return True, "trial", ""
        return False, "free", "исчерпаны free/trial на сегодня"

    def my_stats(uid: int):
        _roll(uid)
        t = {"free": _COUNTS[uid]["free"], "trial": _COUNTS[uid]["trial"], "credit": 0, "sub": 0}
        return {"today": t, "last7": t, "last30": t}

    def daily_summary():
        return {"day": _DAY(), "dau": len(_COUNTS), "free_total": sum(x["free"] + x["trial"] for x in _COUNTS.values()),
                "paid": 0, "credit": 0, "sub": 0}

    def add_credits(uid: int, cnt: int): return 0
    def activate_sub(uid: int, months: int): return False

    def build_buy_keyboard(stars_enabled: bool, card_url: str | None, erip_url: str | None):
        rows = []
        if stars_enabled:
            rows.append([InlineKeyboardButton("⭐ Купити Stars", callback_data="buy_stars:CREDITS_50")])
        if card_url:
            rows.append([InlineKeyboardButton("💳 Оплата картой", url=card_url)])
        if erip_url:
            rows.append([InlineKeyboardButton("🏦 ЕРИП", url=erip_url)])
        if not rows:
            rows = [[InlineKeyboardButton("Скоро доступны способы оплаты", callback_data="noop")]]
        return InlineKeyboardMarkup(rows)

    def apply_payment_payload(uid: int, kind: str) -> str:
        return f"✅ Платёж принят: {kind}. Баланс обновлён (демо)."

    _get_stars_amount_ext = None
...
async def mystats_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = update.effective_user.id
    try:
        s = await asyncio.wait_for(asyncio.to_thread(my_stats, uid), timeout=2.5)
    except Exception as e:
        log.warning(f"/mystats timeout/fail: {e}")
        s = {"today": {"free": 0, "trial": 0, "credit": 0, "sub": 0},
...
async def successful_payment_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    payload = (update.message.successful_payment.invoice_payload
               if update.message and update.message.successful_payment else None)
    if payload:
    ...
    def do_POST(self):
        try:
            ...
            if path == "/vdb/search":
                ...
                try: grade_int = int(grade_in) if grade_in is not None else 8
                except Exception: grade_int = 8
